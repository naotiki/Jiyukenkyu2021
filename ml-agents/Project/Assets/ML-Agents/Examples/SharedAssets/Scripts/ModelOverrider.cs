    ...
    public class ModelOverrider : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            if (m_MaxEpisodes > 0)
            {
                ...
                {
                    Debug.Log(
                        $"ModelOverride reached {TotalCompletedEpisodes} episodes and {TotalNumSteps} steps. Exiting.");
                    Application.Quit(0);
#if UNITY_EDITOR
                    EditorApplication.isPlaying = false;
#endif
        ...
        public NNModel GetModelForBehaviorName(string behaviorName)
        {
            ...
            {
                Debug.Log(
                    $"Couldn't load model file(s) for {behaviorName} in {m_BehaviorNameOverrideDirectory} (full path: {Path.GetFullPath(m_BehaviorNameOverrideDirectory)}");
